...
func PDBxRead(pdb io.Reader, read_additional ...bool) (*Molecule, error) {
...
func PDBxFileRead(pdbname string, read_additional ...bool) (*Molecule, error) {
...
func pdbxBufIORead(pdb *bufio.Reader) (*Molecule, error) {
	...
	for {
		...
		line = strings.ToLower(line) //come on, man.
		if strings.HasPrefix(line, "#") || strings.HasPrefix(line, "#") {
			continue
		}
		if strings.HasPrefix(line, "_atom") {
			reading = true
			field = 0
		}
		if !reading {
		...
		if strings.HasPrefix(line, "_") {
			if !strings.HasPrefix(line, "_atom") { //a new section started
				reading = false
				continue
			}
			m.add(line, field)
			...
			if modkey >= 0 {
				if modkey < len(fields) {
					return nil, fmt.Errorf("pdbxBufIORead: Model field out of range: %d, %v", modkey, fields)
				}
				mod := fields[modkey]
			...
			if err != nil {
				return nil, fmt.Errorf("pdbxBufIORead: Couldn't read %d th bfactors for frame %d: %w", len(bfactors[b])+1, currentmodel, err)
			}
		}
