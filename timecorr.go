...
func mapfunc(t Traj, coord *v3.Matrix, A Atomer, c []float64, f func(c *v3.Matrix, A Atomer) float64) []float64 {
	for err := t.Next(coord); ; err = t.Next(coord) {
		if err != nil {
			if _, ok := err.(LastFrameError); ok {
				break
			}
			panic(err.Error())
		}
...
func MDCorrelation(t1, t2 Traj, A1, A2 Atomer, f1, f2 func(c *v3.Matrix, A Atomer) float64, trjlen int, auto ...bool) []float64 {
	if trjlen == 0 {
		trjlen = 1e6
	}
	...
	ret = CoreCorrMem(c1, c2, c1pad, c2pad, ret) //, ret)
...
func CoreCorrMem(c1, c2 []float64, c1pad, c2pad []complex128, dst ...[]float64) []float64 {
...
func RMSDCorrFunc(refcoord *v3.Matrix, indexes ...[]int) func(c *v3.Matrix, A Atomer) float64 {
	return func(c *v3.Matrix, A Atomer) float64 {
		var f float64
		var err error
		if len(indexes) > 0 {
			f, err = RMSD(c, refcoord, indexes[0], indexes[0])
		} else {
			f, err = RMSD(c, refcoord)
		}
		Err(err)
		return f
	}
